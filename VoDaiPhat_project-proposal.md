...
1\. \*\*USER:\*\* \`id\`, \`email\`, \`password\_hash\`, \`name\`, \`timezone\`.  
2\. \*\*SUBJECT:\*\* \`id\`, \`user\_id\`, \`name\`, \`credits\`, \`target\_score\`, \`color\`.  
3\. \*\*TASK:\*\* \`id\`, \`subject\_id\`, \`title\`, \`description\`, \`status\`, \`priority\`, \`due\_date\`, \`estimated\_minutes\`, \`actual\_minutes\`.  
4\. \*\*STUDY\_SESSION:\*\* \`id\`, \`task\_id\`, \`start\_time\`, \`end\_time\`, \`duration\_minutes\`. \*(Phục vụ Tracking dữ liệu cho Dashboard)\*.

\`\`\`mermaid  
erDiagram  
    USER ||--o{ SUBJECT : "creates"  
    SUBJECT ||--o{ TASK : "contains"  
    TASK ||--o{ STUDY\_SESSION : "has"
    ...
    SUBJECT {  
        ...
        float target\_score "Mục tiêu điểm"  
    ...
    TASK {  
        ...
        datetime due\_date  
        int estimated\_minutes "Dự kiến"  
        int actual\_minutes "Thực tế (Cộng dồn)"  
    }
    ...
    STUDY\_SESSION {  
        ...
        datetime end\_time  
        int duration\_minutes "Thời gian phiên (VD: 25p)"  
    }  
\`\`\`  
\---
...
3\. \*\*Về cấu trúc CSDL:\*\* Việc tách riêng bảng \`STUDY\_SESSION\` để lưu trữ từng phiên Pomodoro (nhằm phục vụ vẽ biểu đồ thống kê sau này) có bị coi là over-engineering cho đồ án môn học không, hay nên gộp thẳng dữ liệu này vào bảng \`TASK\` để tối ưu thời gian query ạ?
